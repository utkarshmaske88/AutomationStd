define([
...
], function (SuperClass, BreaseEvent, Enum, Utils, controllerUtils) {
    ...
    */
    ...
    p._onWidgetsReadyHandler = function () {
        ...
        var childrenWidgets = this.selectChildren();
        this.childrenWidgetIds = [];

        for (var i = 0; i < childrenWidgets.length; i++) {
            if (brease.uiController.getWidgetState(childrenWidgets[i].id) !== 2) {
                this.childrenWidgetIds.push(childrenWidgets[i].id);
            }
        }
        this.elem.addEventListener(BreaseEvent.WIDGET_READY, this._bind('_childWidgetReadyHandler'));
    };

    p._childWidgetReadyHandler = function (e) {
        if (this.childrenWidgetIds.includes(e.target.id)) {
            var childId = e.target.id;
            var childIndex = this.childrenWidgetIds.indexOf(childId);
            this.childrenWidgetIds.splice(childIndex, 1);
        }        
    ...
    p._dispatchOnWidgetsReady = function () {
        this.elem.removeEventListener(BreaseEvent.WIDGET_READY, this._bind('_childWidgetReadyHandler'));
        /**
        * @event OnWidgetsReady
        * @iatStudioExposed
        * Fired when compound and child widgets are ready
        */
        var onWidgetsReady = this.createEvent('OnWidgetsReady');
